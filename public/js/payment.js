...
paypal.Buttons({
  ...
  onInit: function onInit(data, actions) {
    // Disable the buttons
    actions.disable(); // Listen for changes to the checkbox
  ...
  onApprove: function onApprove(data, actions) {
    return fetch('/app/paypal/order/success/' + data.orderID, {
      ...
      return res.json();
    }).then(function (res) {// window.location.href = '/account/paypal/order/success';
    });
  },
  // Wait for the payment to be authorized by the customer
...
0,[["./resources/src/js/payment.js","/js/manifest"]]]);
